...
export const getOverview = async (req: Request, res: Response) => {
    try {
        const [
            usersCount, teachersCount, adminsCount,
            subjectsCount, departmentsCount, classesCount
        ] = await Promise.all([
...
export const getLatestActivities = async (req: Request, res: Response) => {
    try {
        const { limit = 5 } = req.query
        const limitPerPage = Math.min(Math.max(1, parseInt(String(limit), 10) || 5), 100) //max 100 records per page
...
export const getAggregatesForCharts = async (req: Request, res: Response) => {
    try {
